...
class Jupman:
    ...
    def python_tutor(self, **kwargs):
        ...
        # return jupman.pytut(disableHeapNesting=disableHeapNesting, transform=f, text_length=text_length)
        return jupman.pytut(transform=f, **kwargs)
    ...
    def icp(self, content_editable=True, width="55vw", height="65vh"):
        """ Embeds an ICP console in the output of the current Jupyter cell,
            ...
            ICP component authors: Luca De Menego, Lorenzo Angeli
        """

        
        contenteditable = "true" if content_editable else "false"
        ...
        import re
        icp_pattern = r'((conf\.jm\.)?icp\(.*\))'
        
        
        lst = re.findall(icp_pattern, code, re.MULTILINE)
        if len(lst) > 1:
            print(f"ERROR: There should only be one call to jupman.icp(), found {len(lst)} instead")
            return    
        
        m = re.search(icp_pattern, code, re.MULTILINE)
        if m:
            extra = code[m.span()[1]:]
            if len(extra.strip()) > 0:        
                print(f"ERROR: the call to jupman.icp() must be the last in the cell, instead, found this code afterwards: \n{extra}")
                return        
        
        new_code = re.sub(icp_pattern, '', code)
        ...
        if len(new_code.strip()) == 0:
            print("""
            Nothing to show ! You have to put ALL the code IN THE SAME cell as icp()
        ...
        import jupman
        relpath = jupman.detect_relpath(notebook_globals["In"]) 
        
        inject = ""
            
                            
        inject += """
                <div class="jupman-icp-container" style="width:%s; height:%s;">
                <python-editor id="%s" contenteditable="%s" code="%s"
                />
                </div>
        

        """ % (width,height, div_id, contenteditable, new_code)
    ...
    def init_icp(self):
        ...
        inject = ''
        inject +=  """        
            <script src="%s_static/js/python.iife.js" type="application/javascript"></script>
        """ % relpath
